    ...
    void TrainNodesRecurse(std::vector<Node<F, S> >& nodes, NodeIndex nodeIndex, DataPointIndex i0, DataPointIndex i1, int recurseDepth, const IDataPointCollection& data, bool is_parent=false)
    {
      ...
      for(int threadIndex=0; threadIndex < maxThreads_; threadIndex++)
      {
        ...
        for (int f = 0; f < parameters_.NumberOfCandidateFeatures/maxThreads_; f++)
        {
          ...
          F feature = trainingContext_.GetRandomFeature(random_, data_, &indices_[0], (unsigned int)i0, (unsigned int)i1,parameters_.svm_c,parameters_.data_partitions, parameters_.data_partition_types, parameters_.data_partition_lengths, is_parent);
