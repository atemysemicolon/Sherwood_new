...
{
  AxisAlignedFeatureResponse AxisAlignedFeatureResponse ::CreateRandom(Random& random, const IDataPointCollection& data, unsigned int* dataIndices, const unsigned int i0, const unsigned int i1,float svm_c, std::vector<int>& partitions, std::vector<char>& partition_types, std::vector<int>& partition_lengths,  bool root_node)
  {
  ...
  LinearFeatureResponse2d LinearFeatureResponse2d::CreateRandom(Random& random, const IDataPointCollection& data, unsigned int* dataIndices, const unsigned int i0, const unsigned int i1,float svm_c, std::vector<int>& partitions, std::vector<char>& partition_types,std::vector<int>& partition_lengths,  bool root_node)
  {
  ...
  LinearFeatureResponse LinearFeatureResponse::CreateRandom(Random& random, const IDataPointCollection& data, unsigned int* dataIndices, const unsigned int i0, const unsigned int i1,float svm_c, std::vector<int>& partitions, std::vector<char>& partition_types,std::vector<int>& partition_lengths,  bool root_node)
  {
...
void LinearFeatureResponseSVM::GenerateMaskHypercolumnStatistics(Random &random, std::vector<int> &vIndex, int dims,
  ...
  void LinearFeatureResponseSVM::GenerateMaskGrouped(Random &random, std::vector<int> &vIndex, int start, int end, bool root_node)
  {
    //std::cout<<"G : ";
  ...
  void LinearFeatureResponseSVM::GenerateMaskChoiced(Random &random, std::vector<int> &vIndex, int start, int end, int max, bool root_node)
  {
    //std::cout<<"C : ";
    if (max==0)
      max = random.Next(0, end-start);
    int numBloks = random.Next (0, max);
    for(int i = 0;i<numBloks;i++)
    {
        vIndex.push_back(random.Next(start,end));
    }
  }

  void LinearFeatureResponseSVM::GenerateMaskStatistics(Random &random, std::vector<int> &vIndex, int start, int end, int max, bool root_node)
  {
    //std::cout<<"C : ";
    if (max==0)
      max = random.Next(0, end-start);
    ...
    for(int i = 0;i<numBloks;i++)
    {
      int selection = random.Next(start,end);
      vIndex.push_back(selection*2); //Mean
      vIndex.push_back(1+selection*2); //Standard deviation
    }
  }


  LinearFeatureResponseSVM LinearFeatureResponseSVM::CreateRandom(Random& random, const IDataPointCollection& data, unsigned int* dataIndices, const unsigned int i0, const unsigned int i1,float svm_c, std::vector<int>& partitions, std::vector<char>& partition_types, std::vector<int>& partition_lengths, bool root_node)
  {
    ...
    int count_partitions = partitions.size();
    int sum = 0;
    for(int i=0;i<count_partitions;i++)
    {
      char choice = partition_types[i];
      if(choice == 'G')
          GenerateMaskGrouped(random,lr.vIndex_,sum, sum+partitions[i], root_node);
      else if(choice == 'M')
      {
        int  max_choices = partition_lengths[i];
        GenerateMaskStatistics(random, lr.vIndex_, sum, sum+partitions[i], max_choices,root_node);
      }
      else
      {
        int  max_choices = partition_lengths[i];
        GenerateMaskChoiced(random, lr.vIndex_, sum, sum+partitions[i], max_choices,root_node);
      }
    ...
    /*
    for(int bla = 0;bla<lr.vIndex_.size();bla++)
    {
      std::cout<<lr.vIndex_[bla]<<", ";
    }
